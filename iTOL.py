...
def upload(treefile='', zipfile='', treename='', ID='', projectname='', treedescription=''):
    ...
    :return: str, the tree ID generated by iTOL.

    Note:
    Either a treefile or a zipfile is needed, if both of them were assigned, the zipfile will be ignored.
    ...
    if treefile:
        if isinstance(treefile, str):
            if os.path.isfile(treefile):
                ...
                if basename.endswith('.txt'):
                    name = basename[:-4]
                elif basename.endswith('.tree.txt'):
                    name = basename[:-9]
                else:
                    basename = '{}.txt'.format(basename)
                    name = basename
        ...
        if isinstance(zipfile, str):
            if os.path.isfile(zipfile):
                ...
                raise ValueError('Invalid ZIP file: {}.'.format(zipfile))
        else:
            raise TypeError('Argument treefile should be a string.')
    else:
        raise ValueError('Neither treefile nor zipfile was assigned.')
    
    if treename:
        if isinstance(treename, str):
            name = treename
        else:
            raise TypeError('Argument treename should be a string.')
        
    if ID:
        if not isinstance(ID, str):
            raise TypeError('Argument ID should be a string.')
        
    if projectname:
        if not isinstance(projectname, str):
            raise TypeError('Argument projectname should be a string.')
        
    if treedescription:
        if not isinstance(treedescription, str):
            raise TypeError('Argument treedescription should be a string.')

    options = {'treeName': name}
    if ID:
        options['uploadID'] = ID
    else:
    ...
    if treedescription:
        options['treeDescription'] = treedescription

    respond = requests.post(UPLOAD_URL, options, files={'zipFile': open(zf, 'rb')})
    info = respond.text
...
def download(treeID, format='pdf', **kwargs):
    """
    ...
    respond = requests.get(DOWNLOAD_URL, options)
    info = respond.text
    if info.startswith('ERROR'):
        print('\nTree download failed due to the following reason:\n\t{}'.format(info))
        sys.exit(1)
    else:
        outfile = '{}.{}'.format(treeID, format)
